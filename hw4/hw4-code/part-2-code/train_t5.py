...
PAD_IDX = 0 # T5 uses 0 as padding
...
def get_args():
    ...
    parser.add_argument('--learning_rate', type=float, default=5e-5) # Safer LR
...
def clean_sql_query(query):
    """
    Heuristics to fix common syntax errors.
    Adapted from your uploaded 2921d...py file.
    """
    query = query.strip()
    
    # Fix spaces before commas (common T5 artifact)
    query = query.replace(" ,", ",")
    ...
    # Remove potential trailing garbage words if model stopped mid-sentence
    if query.endswith(" AND;"): query = query[:-5] + ";"
    if query.endswith(" WHERE;"): query = query[:-7] + ";"
...
def train(args, model, tokenizer, train_loader, dev_loader, optimizer, scheduler):
    ...
    # *** CRITICAL: Use the cached ground truth records to speed up eval ***
...
def eval_epoch(args, model, tokenizer, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    generation_config = GenerationConfig(
        max_new_tokens=256,       # Increased length
        pad_token_id=PAD_IDX,
        eos_token_id=tokenizer.eos_token_id,
        num_beams=4               # Beam search enabled
    )
    ...
    # Use the cached ground truth records (gt_query_records) to save time
